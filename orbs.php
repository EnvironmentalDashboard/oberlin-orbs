...
require 'db.php';
$result = $db->query('SELECT o.id as orb_id, name,inet_ntoa(ip),ip, disabled, water_uuid, elec_uuid, elec_rvid, water_rvid, o.last_connectioned_on, r1.relative_value as elec_rv, r2.relative_value as water_rv, testing FROM orbs o LEFT JOIN relative_values r1 ON r1.id = o.elec_rvid LEFT JOIN relative_values r2 ON r2.id = o.water_rvid WHERE o.disabled = 0 ORDER BY `name`');
foreach ($result as $row) {
	...
	// 	shell_exec("echo \"^R00B00G00f32+\" | timeout 2s netcat {$row['orb_id']} 9950");
	// }
}
?>
